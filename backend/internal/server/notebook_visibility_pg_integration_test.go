...
func newVisibilityFixture(t *testing.T) visibilityFixture {
	...
	// Two accounts. The no-PII users table stores only google_sub + username
	// (migration 024); the sub doubles as the username here.
	seedUser := func(sub string) int64 {
		var id int64
		require.NoError(t, db.Get(&id,
			`INSERT INTO users (google_sub, username) VALUES ($1, $1) RETURNING id`, sub))
		return id
	}
	ownerID := seedUser("vis-owner")
	nonOwnerID := seedUser("vis-nonowner")
