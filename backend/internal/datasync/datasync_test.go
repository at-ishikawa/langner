				...
				learningSource.EXPECT().FindByNotebookID("wpme").Return([]notebook.LearningHistoryExpression{
					{
						...
						EtymologyOriginLogs: []notebook.LearningRecord{
							{Status: "understood", LearnedAt: notebook.NewDate(baseTime), Quality: 4, ResponseTimeMs: 1500, QuizType: string(notebook.QuizTypeEtymologyOrigin), IntervalDays: 7},
							{Status: "misunderstood", LearnedAt: notebook.NewDate(baseTime.Add(24 * time.Hour)), Quality: 1, ResponseTimeMs: 3000, QuizType: string(notebook.QuizTypeEtymologyOrigin), IntervalDays: 1},
						},
					},
				}, nil)
