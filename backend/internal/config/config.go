...
type OutputsConfig struct {
	...
	// QuizReviewDirectory holds the markdown / PDF files generated by
	// `langner notebooks quiz-review`. One file per notebook with
	// failures on the requested date is written under
	// <QuizReviewDirectory>/<date>/<notebookID>.md. Falls back to
	// StoryDirectory when empty so the new command works without a
	// config change.
	QuizReviewDirectory string `mapstructure:"quiz_review_directory"`
}
