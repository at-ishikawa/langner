...
// the matching YAML slot (LearnedLogs / ReverseLogs / EtymologyOriginLogs).
//
// The previous buildExpression sent ANYTHING not equal to "reverse" into
// LearnedLogs, so a word with etymology_origin_logs in source YAML came
// back with all its etymology logs merged into LearnedLogs on round-trip.
// User-reported as 10 mismatches from `langner migrate validate-db`
// (e.g. gauche source=1 / exported=8, peer source=3 / exported=13).
func TestYAMLLearningRepository_WriteAll_RoutesQuizTypesToCorrectSlots(t *testing.T) {
	...
	logs := []LearningLog{
		...
		{NoteID: 1, Status: "understood", LearnedAt: baseTime.Add(3 * time.Hour), QuizType: "etymology_origin", SourceNotebookID: "demo"},
	}
	...
	assert.Len(t, expr.EtymologyOriginLogs, 1,
		"etymology_origin quiz_type → EtymologyOriginLogs")
}
