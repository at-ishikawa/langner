	...
	return database.RunInTx(ctx, r.db, func(ctx context.Context, tx *sqlx.Tx) error {
		...
		for _, n := range notes {
			if err := tx.GetContext(ctx, &n.ID,
				`INSERT INTO notes ("usage", entry, meaning, level, dictionary_number, concept_key, part_of_speech) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`,
				n.Usage, n.Entry, n.Meaning, n.Level, n.DictionaryNumber, n.ConceptKey, n.PartOfSpeech); err != nil {
...
func (r *DBNoteRepository) Create(ctx context.Context, note *NoteRecord) error {
	return database.RunInTx(ctx, r.db, func(ctx context.Context, tx *sqlx.Tx) error {
		if err := tx.GetContext(ctx, &note.ID,
			`INSERT INTO notes ("usage", entry, meaning, level, dictionary_number, concept_key, part_of_speech) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`,
			note.Usage, note.Entry, note.Meaning, note.Level, note.DictionaryNumber, note.ConceptKey, note.PartOfSpeech); err != nil {
			return fmt.Errorf("insert note: %w", err)
		}
	...
	return database.RunInTx(ctx, r.db, func(ctx context.Context, tx *sqlx.Tx) error {
		for _, n := range notes {
			_, err := tx.ExecContext(ctx,
				`UPDATE notes SET "usage" = $1, entry = $2, meaning = $3, level = $4, dictionary_number = $5, concept_key = $6, part_of_speech = $7 WHERE id = $8`,
				n.Usage, n.Entry, n.Meaning, n.Level, n.DictionaryNumber, n.ConceptKey, n.PartOfSpeech, n.ID)
			if err != nil {
