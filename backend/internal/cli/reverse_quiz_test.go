								...
								Expressions: []notebook.LearningHistoryExpression{
									{
										...
										LearnedLogs: []notebook.LearningRecord{
											{Status: "usable", LearnedAt: notebook.NewDate(time.Now().Add(-365 * 24 * time.Hour))},
										},
									},
								},
							},
			...
			Scenes: []notebook.LearningScene{
				{
					...
					Expressions: []notebook.LearningHistoryExpression{
						{
							Expression:  "eager",
							LearnedLogs: []notebook.LearningRecord{{Status: "understood", LearnedAt: notebook.NewDate(time.Now().Add(-30 * 24 * time.Hour))}},
						},
					},
				},
			},
	...
	require.NoError(t, notebook.WriteYamlFile(filepath.Join(learningNotesDir, "test-fc.yml"), []notebook.LearningHistory{
		{
			...
			Expressions: []notebook.LearningHistoryExpression{
				{Expression: "abstruse", LearnedLogs: []notebook.LearningRecord{{Status: "understood", LearnedAt: notebook.NewDate(time.Now().Add(-30 * 24 * time.Hour))}}},
				{Expression: "break the ice", LearnedLogs: []notebook.LearningRecord{{Status: "understood", LearnedAt: notebook.NewDate(time.Now().Add(-30 * 24 * time.Hour))}}},
			},
		},
	}))
