...
func NewNotebookQuizCLI(
	...
	if notebookName == "" {
		...
		if len(notebookIDs) > 0 {
			cards, err = svc.LoadCards(0, notebookIDs, includeNoCorrectAnswers, nil)
			if err != nil {
		...
		cards, err = svc.LoadCards(0, []string{notebookName}, includeNoCorrectAnswers, nil)
		if err != nil {
			return nil, fmt.Errorf("failed to load quiz cards: %w", err)
		}
	}
...
func (r *NotebookQuizCLI) Session(ctx context.Context) error {
	...
	if err := r.svc.SaveResult(ctx, 0, *currentCard, grade, responseTimeMs); err != nil {
		return err
	}
