...
`), 0o644))
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			// DB save-path helper.
			got := svc.nextIntervalDays(context.Background(), "test-vocab", "", tc.quizType, true, 4, 1200, learnedAt, "break the ice")
...
func TestNextIntervalDays_FirstAttempt(t *testing.T) {
	...
	learnedAt := time.Date(2026, 8, 29, 9, 39, 0, 0, time.UTC)
	got := svc.nextIntervalDays(context.Background(), "test-vocab", "", notebook.QuizTypeReverse, true, 4, 1000, learnedAt, "lose one's temper")
