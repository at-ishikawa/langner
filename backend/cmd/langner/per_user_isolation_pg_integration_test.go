...
// extension of learning-history invariants L1/L4), and each user's scoped read
// (LoadForNotebooks(ctx, ids, userID)) surfaces only their own attempts.
//
...
func TestPerUserHistoryIsolation_LivePostgres_Integration(t *testing.T) {
	...
	// The read side is symmetric and isolated: each user's scoped read
	// reconstructs ONLY their own attempt for the shared note.
	statusFor := func(userID int64) string {
		histories, lerr := repos.HistoryStore.LoadForNotebooks(ctx, []string{target.NotebookID}, userID)
