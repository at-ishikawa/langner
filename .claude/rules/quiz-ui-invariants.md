...
## Worked example — grammar Relearn post re-drills wrong blanks in-session

The grammar Relearn post (`RelearnGrammarPost.tsx`) used to be answered in one pass and dropped: on "Next" it called `completePost(correctCount, blankCount)`, which removed the whole post regardless of how many blanks were wrong, so a blank answered incorrectly was never re-asked in the same session (unlike the single-card screen's `resolveFront`, which pushes a wrong card to the back, and the origin family card's `completeOrigin`, which re-queues wrong words). Users wanted a wrong grammar blank to come back until correct within the session.

Fix — mirror `completeOrigin` at the post level. The store action is now `completeGrammarPost(wrongBlanks, correctCount)`: on "Next" the component reports the blanks whose graded result is not correct (unanswered→incorrect blanks are already graded wrong by "See answers", so they are included), the pass tallies (each blank counts once toward `totalAnswers`; a blank joins `clearedCount` on the pass it is finally correct), and if any blank was wrong a NEW `{ kind: "post", post: { content: sameContent, blanks: wrongBlanks, attempt: attempt+1 } }` is pushed to the BACK — a shrinking post with only the still-missed blanks. `RelearnPostGroup.attempt` is folded into the React `key` (`` `${content} #${attempt}` ``) so the post remounts fresh on retry, exactly like the origin card's `attempt`. When every blank is correct the post is dropped (old behavior).

This is **queue-only** — `SubmitRelearnAnswer` still persists nothing (no learning-history write, no skip, no `skipped_at`, no RPC), so it satisfies U1/U2 the same way the origin re-drill does: a re-drilled blank is simply still due next session like any other Relearn miss. The re-drill only reshapes the current session's working queue.
