...
export default function EtymologyReversePage() {
  ...
  const [feedback, setFeedback] = useState<{
    correct: boolean; reason: string; correctOrigin: string; type: string; language: string;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const fb = { correct: res.correct, reason: res.reason, correctOrigin: res.correctOrigin,
            ...
            <>
              <FeedbackActions isCorrect={displayCorrect} noteId={feedback.noteId} isOverridden={overridden} isSkipped={skipped}
                ...
                onOverride={feedback.noteId ? async () => {
                  try {
                    const res = await quizClient.overrideAnswer({ noteId: feedback.noteId!, quizType: ProtoQuizType.ETYMOLOGY_REVERSE, learnedAt: feedback.learnedAt!, markCorrect: !displayCorrect });
                ...
                onUndo={feedback.noteId ? async () => {
                  try {
                    const res = await quizClient.undoOverrideAnswer({ noteId: feedback.noteId!, quizType: ProtoQuizType.ETYMOLOGY_REVERSE, learnedAt: feedback.learnedAt!, originalQuality: overrideOriginals?.quality ?? 0, originalStatus: overrideOriginals?.status ?? "", originalIntervalDays: overrideOriginals?.intervalDays ?? 0 });
                ...
                onSkip={feedback.noteId ? async () => { try { await quizClient.skipWord({ noteId: feedback.noteId! }); setSkipped(true); storeSkipResult(currentIndex, "etymology-reverse"); } catch {} } : undefined}
              ...
              >
                <Box p={4} borderWidth="1px" borderRadius="lg" bg="white" _dark={{ bg: "gray.800" }}>
                  <Text fontSize="xl" fontWeight="bold">{feedback.correctOrigin} = {card.meaning}</Text>
