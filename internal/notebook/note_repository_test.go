...
func TestDBNoteRepository_FindByNotebook(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "returns notes for notebook",
			notebookType: "story",
			notebookID:   "book-1",
			setupMock: func(mock sqlmock.Sqlmock) {
				noteRows := sqlmock.NewRows([]string{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := repo.FindByNotebook(context.Background(), tt.notebookType, tt.notebookID)
			if tt.wantErr {
				assert.Error(t, err)
				return
			}
...
func TestDBNoteRepository_CreateNotebookNote_LastInsertIdError(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer db.Close()
	...
	err = repo.CreateNotebookNote(context.Background(), nn)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "LastInsertId")
}

func TestDBNoteRepository_Create_BeginTxxError(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer db.Close()
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "BeginTxx")
}

func TestDBNoteRepository_Create_LastInsertIdError(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer db.Close()
