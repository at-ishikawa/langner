...
func TestDBDictionaryRepository_Upsert(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			setupMock: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec("INSERT INTO dictionary_entries").
					WithArgs("hello", "rapidapi", "https://api.example.com", json.RawMessage(`{"word":"hello"}`)).
					WillReturnError(fmt.Errorf("connection refused"))
			},
			wantErr: true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err = repo.Upsert(context.Background(), tt.entry)
			if tt.wantErr {
				assert.Error(t, err)
				return
			}
	...
	tests := []struct {
		...
		},
		{
			...
			setupMock: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec("INSERT INTO dictionary_entries").
					WithArgs(
						"hello", "rapidapi", "https://api.example.com", json.RawMessage(`{"word":"hello"}`),
						"world", "rapidapi", "https://api.example.com", json.RawMessage(`{"word":"world"}`),
					).
